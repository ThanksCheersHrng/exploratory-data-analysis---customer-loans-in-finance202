...
### Methods: 
	...
	Task 3 is about data cleaning by dropping columns with high percentages of nulls (I chose 25% as a threshold) and imputing values; since all remaining (un-dropped) columns with nonzero percentages of null values had asymetric distributions, the median is the better choice between median or mean. I noticed term_length_in_months was essentially binary data, so adding another 8% of the "median" would increase the skew artificially; in an ideal world I would have set up a quick algorithm that generated random selections of either the high or low term lengths according to the distribution (a roughly 7:3 ratio of frequencies) to replace each null value but maintain the binary probability distribution; this comes (as does all imputing) with the caveat that any observable correlations with other variables could be obscured or amplified by pure chance. 
