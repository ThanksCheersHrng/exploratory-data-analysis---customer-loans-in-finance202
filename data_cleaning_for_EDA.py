...
class DataFrameInfo():
    ...
    # Loop through to make sure every pair gets plotted, four scatterplots to a png. 
    for idx, (column1, column2) in enumerate(high_corr_pairs):
        ...
        plt.savefig(f"high_corr_scatterplot_{idx+1}.png") # I would use column names but there are 8 per png file. 
        plt.close()  # Close the current plot to release memory

        # If four scatterplots have been saved into a single png, create a new figure (trying to compromise on memory vs visibility.)
        if (idx + 1) % 4 == 0:
            plt.figure()
